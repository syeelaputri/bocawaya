const womenProductItemContainer = document.getElementById('women-product-item-container');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const womenProductList = productList.filter(product => product.category === 'Pakaian Wanita');

  womenProductList.forEach((menProduct, index) => {
    const delay = (index + 1) * 100;
    const cardItem = createCardItem(menProduct, delay);
    womenProductItemContainer.appendChild(cardItem);
  });
});
