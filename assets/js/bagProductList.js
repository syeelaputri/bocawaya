const bagProductItemContainer = document.getElementById('bag-product-item-container');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const bagProductList = productList.filter(product => product.category === 'Tas');

  bagProductList.forEach((menProduct, index) => {
    const delay = (index + 1) * 100;
    const cardItem = createCardItem(menProduct, delay);
    bagProductItemContainer.appendChild(cardItem);
  });
});
