const shoeProductItemContainer = document.getElementById('shoe-product-item-container');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const shoeProductList = productList.filter(product => product.category === 'Sepatu');

  shoeProductList.forEach((menProduct, index) => {
    const delay = (index + 1) * 100;
    const cardItem = createCardItem(menProduct, delay);
    shoeProductItemContainer.appendChild(cardItem);
  });
});
